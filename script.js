...
function renderizarCanciones() {
    ...
    let html = '';
    CANCIONES_DATA.forEach((song, index) => {
        if (song.activo === false) return;

        html += `
        <div class="card" id="cancion${song.id}">
            <div class="state-hidden" id="hidden${song.id}">
                <i class="fa-solid fa-music music-icon-big"></i>
                <button class="btn-reveal" onclick="revelarCancion('${song.id}')">
                    <i class="fa-solid fa-eye"></i>
                </button>
            </div>
            <div class="state-revealed" id="revealed${song.id}" style="display: none;">
                ...
                <div class="card-content">
                    <div class="card-header-top">
                        ...
                        <button class="btn-golden-ticket" onclick="togglePaseOro(this)" title="Dar Pase de Oro">
                    ...
                    <div class="scoring-row">
                        <button class="btn-yellow-small"
                            onclick="abrirLetra(CANCIONES_DATA[${index}].titulo, CANCIONES_DATA[${index}].letra)"
                            title="Ver letra">
...
function revelarCancion(id) {
    ...
    const revealedEl = document.getElementById('revealed' + id);

    if (card && hiddenEl && revealedEl) {
        // 1. Agregar clase de fade-out y barrido a la capa oculta
        hiddenEl.classList.add('fade-out');

        // 2. Agregar la clase .efecto-revelar al contenedor de la tarjeta (para giro 3D y flash de neón)
        card.classList.add('efecto-revelar');

        // 3. Mostrar la capa revelada de la canción (los elementos internos se animarán en cascada vía CSS)
        revealedEl.style.display = 'flex';

        // 4. Ocultar físicamente la capa oculta tras completarse la animación de fade-out (1.8s)
        setTimeout(() => {
...
function empezarFaseFinal() {
    ...
    for (let i = 1; i <= 15; i++) {
        ...
        let finalScoreBadge = card.querySelector('.final-score-badge');
        let notaFinal = finalScoreBadge ? (parseFloat(finalScoreBadge.innerText) || 0) : 0;
    ...
    // 2. Reproducir el sonido de tambores
    ...
    // 3. Inicializar la fase final barajando y rellenando las cajas
    inicializarFaseFinal(clasificados);
}

// Algoritmo de Fisher-Yates para barajar los finalistas de forma aleatoria
function barajarFinalistas(array) {
...
// Inicializa las 8 cajas iniciales barajando las canciones
function inicializarFaseFinal(lista) {
    // Barajar los finalistas y actualizar la variable global
    clasificados = barajarFinalistas([...lista]);

    // Rellenar las cajas con la animación escalonada
    for (let j = 0; j < 8; j++) {
